...
- [ ] `templates/exposure/dagster-webserver-exposed.yaml` — Dagster (webserver OSS, orchestrateur de pipelines ML) : `GET /server_info` (alias historique `/dagit_info`) renvoie `{"dagster_webserver_version":...,"dagster_version":...,"dagster_graphql_version":...}`. Vérifié dans `python_modules/dagster-webserver/dagster_webserver/webserver.py` : `webserver_info_endpoint` renvoie cette `JSONResponse` sans aucune dépendance d'authentification, et la documentation officielle (« Running Dagster locally ») liste explicitement « Authentication or web security features (`dg dev` does not include these) » comme absentes du produit OSS. Le trio de versions est propre à Dagster et confirme que l'endpoint `/graphql` — qui permet de lancer des runs — est atteignable. Matcher sur `dagster_webserver_version` + `dagster_graphql_version`. Sévérité high.
